...
Developer map for the Bram Discord bot. The application lives entirely in `bot/`; `docs/` holds the canonical documentation set. This is the single file to extend. Code paths below are relative to `bot/` unless written otherwise.

## What This Is

A Python 3.14 Discord bot: a generalist assistant for Discord communities, named Bram by default. The name comes from `BOT_NAME`/`settings.bot_name`; never hardcode it. Ordinary guild message turns start from an @mention, a reply with the reply ping on (ping off is ignored), a `hey/hi <bot_name>` or `<bot_name> help` text invocation, or an unmentioned message inside an auto-responding thread it created via thread handoff. Slash commands and context menus are separate explicit invocations. Paused managed threads use ordinary invocation rules. The bot ignores DMs unless `USER_APP_DM_ENABLED` is on, which routes a DM from an allowlisted user into personal chat. It runs a provider-neutral ReAct tool loop, and replies with Discord-safe chunking and attachments. Trust-tiered tools, script-backed skills, per-user workspaces, SQLite-backed conversation persistence, optional Hindsight memory.
...
Run project commands from `bot/` (CI uses it as the working directory). Follow
`docs/development.md` to create `.venv`, then invoke installed Python tools
through that environment's explicit paths. Invoke maintainer-only `uv`, plus
`npm`, `node`, and `git`, directly as shown below. Bare
`python`/`pytest`/`mypy` commands may select another environment.

```bash
.venv/bin/python bot.py                       # run the bot
ENV_FILE=.env.dev .venv/bin/python bot.py     # dev instance: test guild + own DB (docs/development.md)
...
The Activity dashboard frontend in `dashboard/` has its own checks, run from that
directory with Node 22.18 or newer: `npm ci`, `npm audit`, `npm run build`,
`npm test` (Vitest), and `npx playwright install chromium && npm run test:browser`
(Playwright at desktop and phone sizes against a mocked API fixture).

That block uses POSIX paths. In Windows PowerShell, map `.venv/bin/python` to
`.\.venv\Scripts\python.exe`, `.venv/bin/ruff` to
...
- `modules/` implements the module API services (the standalone contract package is
  `packages/bram-agent-module-api`; `modules/testing.py` is the core integration
  ...
  `app/tools.py`.
- Failure behavior is explicit: privilege gates and credential-gated registration fail closed. Pin loaders are permissive: unreadable, malformed, or non-list input yields no pins, while invalid list entries are dropped. Channel, guild, and deployment denylists retain a last-known-good value after one has loaded, while an invalid initial load raises; per-tool config retains a last-known-good value and otherwise uses schema defaults. Startup validation for models, the deployment-wide tool policy, `settings.md`, and the executable-skill sandbox aborts rather than degrades.
...
Guild messages flow through `BramApplication.on_message` (`app/runtime.py`) → `app/message_runtime.py:DiscordMessageController` for eligibility, invocation, block, stop, send-permission, admission, privacy, consent, rooted-conversation, and root-lock gates → `app/foreground_turn.py:ForegroundTurnRunner` with `app/guild_turn_adapter.py:GuildMessageTurnAdapter` → `agent/turn.py`. Personal `/chat` flows through its registered command → `app/user_app_chat.py:UserAppChatController` for access, block, consent, admission, privacy, and root-lock gates → `ForegroundTurnRunner` with `app/user_app_turn_adapter.py:UserAppInteractionTurnAdapter` → `agent/turn.py`. Shared interaction consent lives in `app/user_app_consent.py`; `/stop`, reset, and privacy work cancellation coordinate through `app/work_cancellation.py`. Both turn surfaces then use turn wiring (`app/turn_entry.py`) → `agent/turn.py:handle_turn` (`prepare_turn`, `execute_turn`) → `agent/core.py:run_conversation` (ReAct loop) → provider `run_turn` → tool dispatch via `tools/registry.py`; the surface adapter owns Discord delivery.

Turns are stateless: no provider continuation, no in-process context cache. Each fresh ordinary guild invocation creates a rooted conversation keyed by the triggering message id; `message_contexts` maps both user trigger ids and bot reply ids back to that root so replies continue the same transcript across restarts. Personal `/chat` and enabled allowlisted DMs instead reuse the stable `userchat:<user_id>` root, private to that user, across locations. `agent/context.py` builds a fresh `ConversationContext` each turn from persisted SQLite rows; reply chunks persist back through `ConversationStore.save_channel_messages`, deduped by Discord message id. The response lock is per root: different roots run in parallel, concurrent replies to one root serialize. Every conversational entry point receives the same `RepositoryBlockedUserCheck` (`/stop` and `/privacy` stay deliberately exempt, so a blocked user can still stop work and delete their data): `on_message` before reactions, transcript writes, locks, tools, or provider calls; `/chat` and the teach context menu after their own access/tier checks but before deferring; the coding-task scheduler right after claiming a task, cancelling it with a neutral message instead of running it (`tests/test_blocked_users.py` drives the interactive paths, `tests/test_coding_tasks.py` the claim). `block_user` is a member-tier self-block only, staff block others via `/moderation`.
...
`providers/base.py:LLMProvider` is the abstract interface; every general provider normalizes into `ProviderResponse`/`ToolCall`. The catalog accepts the profile types in `config/model_config.py:SUPPORTED_PROVIDER_PROFILE_TYPES`: `providers/factory.py:SUPPORTED_PROVIDER_NAMES` supplies `openai_compat` (Chat Completions, default), `openai_responses` (stateless Responses API, `store=false`), `anthropic` (native SDK, api.anthropic.com only), `anthropic_compat` (Messages-over-HTTP for gateways), `openrouter`, `codex` (WebSocket Responses transport in `codex/`), and `xai` (native xAI Responses with strict OAuth/API auth modes); the video lifecycle separately constructs the specialized `gemini_interactions` transport.

`config/models.yaml` is **untracked instance state**; the tracked template is `config/models.example.yaml` and a missing live file fails startup rather than falling back. Roles `chat` and `compaction` are required; `chat_images`, `persona`, the specialized `video` role, and the independent durable-worker role `coding` are optional. `coding` never inherits from `chat`, and its primary and `coding_fallbacks` must declare `text` and `tool_calling`; `video` must declare `video_input`, use `gemini_interactions`, and has no fallback chain because stored Interactions pin both model identities. Each model declares a conservative `context_window` and capabilities, and routing rejects a model that cannot satisfy the turn. General roles may declare `<role>_fallbacks`; `ProviderManager.resolve` returns a `providers/failover.py:FailoverProvider` for single and multi-model chains. Transient errors get one retry, fallback is forward-only and sticky within a logical turn, and SQLite-backed circuits skip unhealthy model/account scopes across turns and restarts. Generic policy lives in `providers/failure_policy.py`; optional adapters normalize provider-specific structured errors. `openai_compat` streams with a stall watchdog (`PROVIDER_STREAM_STALL_TIMEOUT_SECONDS`, 90s); the whole-turn ceiling is `REACT_TURN_TIMEOUT_SECONDS` (3600s). The bot owner's `/models` menu selects among up to 120 `selectable_chat_models`, shows circuits, and can reset all cooldowns; catalog edits still need a restart. See [`docs/providers.md`](docs/providers.md) and [`docs/provider-resilience.md`](docs/provider-resilience.md).

New general provider: subclass `LLMProvider`, implement `run_turn`, declare `capabilities`, add a factory case and a `SUPPORTED_PROVIDER_NAMES` entry, document in `docs/providers.md` and the models template. A specialized lifecycle-owned profile instead belongs in `SPECIALIZED_PROVIDER_NAMES` with a strict role and field contract.
...
- `pinned_tools:` frontmatter pre-activates searchable tools (guild fragment as base, channel fragment unions on, and the selected `dashboard` command template unions on for dashboard turns), read fresh each turn and never persisted. Pinning never widens privileges.
- `blocked_tools:` frontmatter is the denylist at three scopes: `config/tools.md` (deployment-wide, validated at `build_app`, last-known-good on bad reload), guild, and channel; dashboard turns add the selected `dashboard` command template's list (`config/fragments/dashboard.py`, last-known-good on bad reload, fails the turn if never valid). `app/turn_entry.py` unions them and stashes the result on `ctx.blocked_tools`. The only way to *subtract* a global tool from a guild or channel; the denylist beats a pin.
...
`build_discord_embed`, `move_to_thread`, and `leave_thread` replace members of the same frozen `TurnOutbox`. That value rides the final reply as `MessageContext.outbox` → `ConversationContext.pending_outbox` / `ConversationRunResult.outbox` → `TurnResult.outbox`; the guild and user-app adapters read it at the delivery boundary, which re-checks every live gate before posting. Keep new final-reply artifacts inside this value instead of adding another copied rail. The pause/resume thread tools write state immediately instead. Thread tools live in `tools/threads.py` and are gated on `THREAD_HANDOFF_ENABLED`; embeds live in `tools/embeds.py`. Bot-created threads are enrolled in `thread_conversations`, share the rooted transcript and lock, and can auto-respond or pause. `@everyone`/`@here` and pings are hard-blocked at the send layer via `AllowedMentions`, not by prompt. See [`docs/embeds.md`](docs/embeds.md) and [`docs/thread-handoff.md`](docs/thread-handoff.md).

### Web Search, X Search, and Wolfram|Alpha

`internet_search` (`tools/internet_search.py`, core, `MEMBER`) registers when any of `TINYFISH_API_KEY`, `EXA_API_KEY`, or `BRAVE_API_KEY` is set. Backends live in `search/` (`tinyfish.py`, `exa.py`, `brave.py`) behind `search/chain.py`, which blends every configured provider or fails over in rank order according to the `strategy` field in `config/tools/internet_search.md`; provider names, credentials, and cost never reach the model. `x_search` (`tools/x_search.py`, searchable, `MEMBER`) is independent of chat routing, gated on `X_SEARCH_ENABLED`, and authenticates through `xai/` with a strict `X_SEARCH_AUTH_MODE` (`oauth` from the `scripts/xai_auth.py` token file, `api_key` from `GROK_API_KEY`, or OAuth-first `auto`); `providers/xai.py` reuses the same package for the `xai` provider type. `wolfram_alpha` (`tools/wolfram_alpha.py`, searchable, `MEMBER`) registers on `WOLFRAM_ALPHA_APP_ID`. Per-turn budgets are `INTERNET_SEARCH_MAX_BACKEND_CALLS_PER_TURN`, `X_SEARCH_MAX_CALLS_PER_TURN`, and `WOLFRAM_ALPHA_MAX_CALLS_PER_TURN`; billed backends write `paid_usage_ledger` rows, and every result is framed as untrusted context. See [`docs/internet-search.md`](docs/internet-search.md), [`docs/x-search.md`](docs/x-search.md), and [`docs/wolfram-alpha.md`](docs/wolfram-alpha.md).
...
- **Privacy consent gate** (`app/consent.py` for messages, `app/user_app_consent.py:UserAppConsentPrompter` for `/chat` and **Teach <bot>**, `PRIVACY_CONSENT_ENABLED`, off by default): prompts on first interaction before the lock, persistence, or any provider call. Fails closed. `/privacy` (`commands/privacy_cmd.py`) shows the TL;DR plus **Delete memory** and **Delete my data**; full deletion takes an exclusive per-user barrier. See [`docs/privacy.md`](docs/privacy.md) and [`docs/privacy-policy.md`](docs/privacy-policy.md).
...
- **Application modules** (`app/modules.py`, `BRAM_MODULES`): separately installed, explicitly enabled packages may add commands, listeners, database migrations, background work, guild-config validators, and optional LLM tools. Core also provides actor-scoped, staff-approved guild fragment proposals through `ctx.proposals`. A configured module is required and fails startup if missing or incompatible. Module-owned behavior and retention belong in that module's documentation. See [`docs/modules.md`](docs/modules.md).

### Discord Activity Dashboard

Optional per-user private saved chats delivered through a Discord Embedded Activity (`DASHBOARD_ENABLED`, off by default, plus `dashboard: {enabled: true}` in the guild fragment, read on access). `app/dashboard.py` is the same-process HTTP and WebSocket boundary behind an operator HTTPS proxy; it refuses to start without a frontend build and `DASHBOARD_CLIENT_SECRET`. `app/dashboard_auth.py` exchanges the SDK OAuth code, verifies the Activity instance server-side, and keeps in-memory sessions behind a Secure/HttpOnly/SameSite=None/Partitioned cookie; client-supplied identity is never authority. `app/dashboard_access.py` re-checks guild policy, `DASHBOARD_ALLOWED_USER_IDS`, `DASHBOARD_MIN_TIER`, blocks, and live channel permissions on every request. `app/dashboard_turn.py` runs chats through `ForegroundTurnRunner` with the `dashboard` command template (`config/prompts/commands/dashboard.md`, shipped fallback when an external `CONFIG_DIR` lacks it); `app/dashboard_files.py` owns bounded private snapshots and inert previews; `app/dashboard_tasks.py` provides private coding delivery (`coding_tasks.delivery_surface`, never falls back to a channel) and scheduled-task approval through `TaskApprovals.decide`. Storage is `storage/dashboard.py` (chats, idempotent turns/actions, replay journal, file records) and `storage/dashboard_branches.py` (branch snapshots, 1,000-message cap); rows cascade with the parent conversation and unfinished work is marked interrupted at startup. Dashboard chats share the ordinary `(user, guild)` workspace, and `BramCommandTree.sync` in `app/runtime.py` appends the type-4 entry point to the same global command replacement. The React frontend lives in `dashboard/` and is served from its `dist/` build. See [`docs/dashboard.md`](docs/dashboard.md) and [`docs/dashboard-cloudflare.md`](docs/dashboard-cloudflare.md).

### Storage

`storage/db.py` is async SQLite (`aiosqlite`, WAL), connected by `app/lifecycle.py` during `on_ready`; `app/lifecycle.py:AppRepositories` owns the database-backed stores from composition onward. Optional SQLCipher is enabled via `DATABASE_ENCRYPTION_KEY`. `messages` is the per-root transcript (one row per Discord message, deduped by `(conversation_id, discord_message_id)`); `message_contexts` maps ids to roots; `usage_ledger` and `paid_usage_ledger` feed `/usage`; `provider_circuits` persists safe normalized cooldown state; `dashboard_*` tables hold Activity chat state and cascade with their conversation. Transcripts age out after `transcript_retention_days` (30) via the retention sweeper; usage ledgers, circuit state, and Hindsight memory are excluded. Module schemas have independent versions in `module_schema_versions`. See [`docs/database.md`](docs/database.md).
