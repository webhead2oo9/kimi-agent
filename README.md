...
Turns are stateless, and each conversation is keyed to the message that
started it and persisted in SQLite, so a reply continues its own thread even
across restarts. Everything is gated by config:
