...
Turns are stateless: there's no provider continuation state. Each fresh mention
opens a rooted logical conversation keyed by the triggering message id, and the
transcript is rebuilt from persisted SQLite rows each turn. The full flow lives
