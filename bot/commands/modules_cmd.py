...
def render_manifest(
    ...
    for name, spec in specs.items():
        ...
        if spec.activation_capabilities:
            lines.append("  activation capabilities: " + ", ".join(spec.activation_capabilities))
        if spec.provides:
        ...
        if perms.scheduled_results:
            lines.append(
                f"  scheduled results: {', '.join(perms.scheduled_results)} (guild-scoped)"
            )
        if perms.tool_files:
