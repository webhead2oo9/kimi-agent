...
export function WorkPanel(props: Props) {
  ...
  const plan = [...events].reverse().find(event => event.kind === "plan")?.payload.steps;
  ...
  return <aside className="work-panel" aria-label="Work panel">
    <header className="work-header"><div><span className="eyebrow">ALONGSIDE YOUR CHAT</span><h2>{selectedFile ? "File preview" : "Your work"}</h2></div><button className="icon-button" aria-label="Close work panel" onClick={onClose}><X size={19} /></button></header>
    {selectedFile ? <FilePreview file={selectedFile} connection={connection} onBack={() => onSelectFile(null)} /> : <div className="work-scroll">
      {!chat && <div className="work-empty"><FolderOpen size={30} /><h3>Room for what’s next</h3><p>Files, plans, and task progress will gather here as you work.</p></div>}
      ...
      {work.coding.map(event => <CodingCard key={event.payload.id} event={event} openLink={connection.openLink} onAction={onAction} />)}
      {!!plan?.length && <section className="work-section plan-section"><h3><Check size={15} />Current plan</h3><ol className="plan">{plan.map((step, index) => <li key={index} className={step.status}><span className="step-marker">{step.status === "completed" ? <Check size={12} /> : index + 1}</span><span>{step.content}</span></li>)}</ol></section>}
      {chat && <section className="work-section"><h3><FileText size={15} />Files in this chat<span>{files.length}</span></h3>{files.length ? <div className="panel-files">{files.map(file => <FileButton key={file.id} file={file} onSelect={onSelectFile} />)}</div> : <p className="muted small-text">Attach a file or ask {connection.session.bot_name} to make one.</p>}</section>}
      {chat && <section className="work-section workspace-section"><button className="section-toggle" aria-expanded={workspaceOpen} onClick={() => workspaceOpen ? setWorkspaceOpen(false) : void browse(directory)}><FolderOpen size={16} /><strong>Your server workspace</strong><ChevronRight size={15} className={workspaceOpen ? "down" : ""} /></button><p className="muted small-text">The same files you use with “hey kimi.”</p>{workspaceOpen && <div className="workspace-browser"><div className="workspace-path"><button className="icon-button" disabled={!directory || loading} aria-label="Parent folder" onClick={() => void browse(directory.split("/").slice(0, -1).join("/"))}><ArrowLeft size={15} /></button><span>{directory || "Your files"}</span><button className="icon-button" aria-label="Refresh workspace" disabled={loading} onClick={() => void browse(directory)}><RefreshCw size={14} /></button></div>{loading && <p role="status">Opening files…</p>}{!loading && workspaceFiles?.length === 0 && <p className="muted small-text">This folder is empty.</p>}{!loading && workspaceFiles?.map(file => <button key={file.path} className="workspace-file" onClick={() => {
...
function FilePreview({ file, connection, onBack }: { file: FileRecord; connection: Connection; onBack: () => void }) {
  ...
  return <div className="preview-shell"><button className="text-button preview-back" onClick={onBack}><ArrowLeft size={14} /> Back to work</button><div className="preview-filename"><FileText size={21} /><div><strong>{file.filename}</strong><small>{readableSize(file.size)}</small></div></div><a className="download-button" href={`/api/files/${file.id}/content`} download={file.filename}><Download size={15} /> Download original</a><div className="preview-content">
...
function ScheduledCard({ preview, status, pending, openLink, onAction }: { pending: boolean; preview: TaskPreview; status: string; openLink: Connection["openLink"]; onAction: Props["onAction"] }) {
  ...
  return <section className="task-card"><div className="card-label"><span className={`status-dot ${status === "approved" ? "complete" : ""}`} />SCHEDULED TASK<span className="pill">Revision {preview.revision}</span></div><h3>{preview.name}</h3><span className="task-status">{status === "pending" ? "Ready for your review" : status}</span><Markdown text={preview.text} openLink={openLink} /><details><summary>Full task details</summary><Markdown text={preview.details} openLink={openLink} /></details><details><summary>Instructions · SKILL.md</summary><pre>{preview.skill}</pre></details>{preview.python !== null && <details><summary>Python source · task.py</summary><pre>{preview.python}</pre></details>}{error && <p role="alert" className="danger-text">{error}</p>}{status === "pending" && <div className="approval-actions"><button disabled={busy || pending} onClick={() => void action("test")}><Play size={14} />Test preview</button><button className="approve" disabled={busy || pending} onClick={() => void action("approve")}><Check size={14} />Approve</button><button disabled={busy || pending} onClick={() => void action("reject")}>Reject</button></div>}</section>;
}

function CodingCard({ event, openLink, onAction }: { event: ChatEvent; openLink: Connection["openLink"]; onAction: Props["onAction"] }) {
  ...
  return <section className="task-card coding-card"><div className="card-label"><Code2 size={15} />CODING TASK<span className="pill">{(payload.status || "").replaceAll("_", " ")}</span></div><Markdown text={payload.text || "Working…"} openLink={openLink} />{active && <form className="steering-form" onSubmit={event => { event.preventDefault(); void act("steer"); }}><label>{payload.status === "waiting_for_input" ? "Your answer" : "Add a direction"}<textarea value={answer} onChange={event => setAnswer(event.target.value)} placeholder={payload.status === "waiting_for_input" ? "Tell Kimi how to continue…" : "Something to keep in mind…"} maxLength={8000} rows={2} /></label><div><button className="text-button" type="button" disabled={busy} onClick={() => void act("cancel")}><Square size={12} />Stop task</button><button type="submit" className="primary" disabled={busy || !answer.trim()}><Send size={13} />Send</button></div></form>}{error && <p role="alert" className="danger-text">{error}</p>}</section>;
}
