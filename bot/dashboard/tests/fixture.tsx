...
api.request = async <T,>(path: string, method = "GET", body?: unknown): Promise<T> => {
  const data = body as { text?: string; title?: string; event_id?: number } | undefined;
  ...
  if (path === "/chats" && method === "POST") { const chat = { ...chats[0], id: crypto.randomUUID(), title: "New chat", parent_id: null, parent_title: null }; chats.unshift(chat); histories.set(chat.id, []); result = chat; }
    ...
    histories.set(branch.id, (histories.get(chatId) || []).filter(event => event.id <= data!.event_id! && ["user_message", "turn_finished", "history_message"].includes(event.kind)).map(event => ({ id: nextEventId++, kind: "history_message", created_at: now, payload: { role: event.kind === "user_message" ? "user" : event.payload.role || "assistant", text: event.payload.text, files: event.payload.files } })));
    const notice = { id: nextEventId++, kind: "branch_created", created_at: now, payload: { chat_id: branch.id, title: branch.title } };
    histories.get(chatId)!.push(notice); subscriptions.get(chatId)?.([notice]);
    result = branch;
  }
    ...
    const update = [{ id: nextEventId++, kind: "user_message", created_at: now, payload: { turn_id: turn, text: data?.text } }, { id: nextEventId++, kind: "turn_finished", created_at: now, payload: { turn_id: turn, status: "completed", text: "That sounds good. I've kept your direction with this conversation." } }];
    histories.get(chatId)!.push(...update); subscriptions.get(chatId)?.(update); result = { turn_id: turn };
  }
  else if (method === "PATCH") { if (current && data?.title) current.title = data.title; }
  else if (method === "DELETE") { const index = chats.findIndex(chat => chat.id === chatId); if (index >= 0) chats.splice(index, 1); }
