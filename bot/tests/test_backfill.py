...
def test_collect_channel_context_orders_chronologically_and_formats() -> None:
    bot_user = FakeAuthor(999, "Bram", bot=True)
    ...
    assert [b.transcript_line for b in result] == ["Alice: hello", "Bram: hi there"]
...
def test_collect_channel_context_appends_reactions_for_assistant() -> None:
    ...
    assert result[0].transcript_line == f"Bram: {expected}"
