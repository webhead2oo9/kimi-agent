...
`app/` is a large package. The important files are `runtime.py` (`build_app`, boot, `on_message`), `foreground_turn.py` (the typed prepare/run/deliver seam), `user_app_turn_adapter.py` (deferred `/chat` delivery), `turn_entry.py` (who gets a turn), `tools.py` (what tools get wired), `modules.py`, and `plugins.py`.
