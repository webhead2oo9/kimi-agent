...
`app/` is a large package. The important files are `runtime.py` (`build_app`, boot, and ingress gates), `command_sync.py` (READY-cohort command publication), `guild_activation.py` (live guild activation and refresh), `foreground_turn.py` (the typed prepare/run/deliver seam), `guild_turn_adapter.py` (gateway-message delivery and handoffs), `user_app_chat.py` (personal-chat policy and execution), `user_app_consent.py` (shared interaction consent), `user_app_turn_adapter.py` (deferred `/chat` delivery), `work_cancellation.py` (foreground/coding teardown), `coding_tasks.py` (durable worker scheduling), `coding_delivery.py` (durable Discord projection and control), `root_locks.py` (refcounted conversation serialization), `turn_entry.py` (who gets a turn), `tools.py` (what tools get wired), `modules.py`, and `plugins.py`.
