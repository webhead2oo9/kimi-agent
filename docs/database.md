...
- `_SCHEMA_SQL` and the core schema helpers build the complete schema for an empty database. The ordered `_MIGRATIONS` registry applies the permanent v8–v16 changes after the v7 baseline.
...
v14 `assistant_dashboard`, v15 `dashboard_branches`, and v16 `scheduled_results`.
The v16 migration adds durable module subscriptions and result acknowledgements;
it does not replay historical task results. Existing v7–v15
...
transcript-format conversion have been removed. A database at v7 through v15
