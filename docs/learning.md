...
not a tool of its own: the model routes each piece of knowledge to one of two
existing, individually tier-gated sinks, which keeps the sink visible in the
tool log instead of hidden behind a wrapper.
...
**In conversation.** Pure prompting: the learn bullets under `## Behavioral
Rules` in `config/prompt.md` tell the model to classify the knowledge, check
the `<skills>` index and `recall_community` first, prefer `skill_edit`
(`append` or `edits`) over a near-duplicate `skill_create`, and then state in
one line what was stored and where so staff can correct it immediately.

**The "Teach Kimi" message context menu** (the name follows `BOT_NAME`;
`commands/learn_cmd.py`) captures a good explanation at its source. It is
staff-gated at the interaction, deferred and answered ephemerally, and refuses
bot-authored and empty messages.

## The context-menu turn

The context menu runs `app/learn_turn.py:run_learn_turn`, never
`handle_message`. That path resolves trust from the message's author (wrong
here: the actor is a different person), persists a transcript, and replies
in-channel.
...
reload; a snapshot denylist could not promise isolation, since it can only
...
it every audit card for a context-menu learn silently loses its source link.

The turn is capped at `LEARN_TURN_TIMEOUT_SECONDS` (a module constant in
`app/learn_turn.py`, 600s), comfortably under Discord's 15-minute
...
(`command_template="learn"`), per-guild overridable at
`config/prompts/commands/learn/<guild_id>.md`, falling back to the normal
template chain if removed. See the
[full-template guide](../bot/config/prompts/README.md). The template leaves out the
content-rating line: a learn turn writes knowledge, it does not chat with
members.

## Hostile messages (accepted risk)

Teaching takes a member's words and may turn them into a skill, and a skill is
read back as instructions on later turns. So the thing to worry about is not a
bad answer once. It is a member writing a message designed to be taught, staff
teaching it in good faith, and the bot carrying those instructions until
somebody notices and deletes them.
...
- The staff member's own note is the one thing outside the fence that the
  prompt says to follow. That distinction is the point: staff are giving
...
That is framing, and framing can be argued with. A determined message that
reads as sincere advice is exactly the case none of it catches.

The sturdier design would be a drafting turn with no tools at all, showing
staff the exact text and writing it only after they confirm. It was considered
and not built. What makes that trade acceptable is everything around the
feature rather than the framing itself: only STAFF can trigger a learn, the
reply names exactly what was stored and where, and every write is posted to the
audit channel, so a bad skill is visible to the people who can remove it.
...
patched, or replaced text for an edit. A card saying only "Skill updated" is
useless for review.

`app/learn_log.py` renders the event to the guild's `learn_log_channel_id`
(guild fragment frontmatter, validated in `server_setup_activation`, fails
closed when absent). It bounds every part and the 6000-character aggregate,
because `discord_adapter/io.py:build_embed` truncates nothing and an oversized
card is rejected by Discord and swallowed. Because the confirmation is
ephemeral, that channel is the only shared record a learn happened, so the
conversational path logs there too, not just the context menu.
...
module; that edge is an import-order cycle, frozen in
`tests/test_package_graph.py`).

`emit_learn_event` takes a *factory*, not a finished event, so that constructing
the event sits inside the same guard as delivering it. Both run after the write
is committed, where an escaping exception would report failure for a write
that actually succeeded and invite a duplicate retry.
