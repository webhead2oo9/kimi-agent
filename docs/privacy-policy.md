...
> set in `PRIVACY_POLICY_URL`. This template covers Bram core only. Separately
> installed application modules must publish their own privacy notice.

Bram is a general-purpose assistant bot for Discord communities. This page
explains, in plain language, what data Bram handles when you talk to it, who
that data is shared with, how long it is kept, and the controls you have. The
technical version is [`privacy.md`](privacy.md).

## TL;DR

- Bram starts an AI conversation when you call on it: a mention, a reply
  with the ping on, `hey Bram` / `hi Bram`, `Bram help`, a message in one of its
...
- Your messages to Bram go to the AI provider that powers its replies. Optional
  features (search, video, images, and so on) send only what that feature
  needs. If you ask Bram to browse a site or run code with network access, the
...
- `/privacy` deletes Bram's local conversation history, workspace files,
  persistent browser profile, video sessions, enabled operator-plugin data,
  and personal memory. For video
  sessions Bram also requests deletion of every known Gemini Interaction and
  uploaded File; it cannot erase Discord messages or guarantee removal from a
  provider's safety logs or backups.

## When Bram is listening

Bram starts an interactive conversation when you call on it: by @mentioning it,
replying with the ping on, starting a message with `hey Bram`, `hi Bram`, or
`Bram help`, or posting in an auto-responding thread it started. In a paused
...
While it works on something you asked for, Bram may read recent messages from
...
does not turn those messages into Bram conversation history just because they
were retrieved. If your invoked message contains a Discord message link, Bram
...
**Bram ignores ordinary direct messages by default.** An operator can enable
...
Depending on the features a server enables, Bram handles:

- **Your messages to it**: the text you send when you invoke Bram, and any
  images or files you share with it in that message.
- **Basic Discord identifiers**: your user ID and display name, and the
  server/channel the message was in, so Bram can reply in the right place and
  keep track of a conversation. If you ask Bram to start a managed thread, it
  also stores your user ID as that thread's initiator so that only you or a
  moderator can close it or change its reply mode.
- **Your server profile, when someone asks about you**: Bram can look up a
...
- **Video session details, if video understanding is enabled**: where the
  ...
  session and in which conversation, and timestamps. Bram does not keep a copy
  of the video, the Discord link to it, your questions, or the answers.
- **Messages in channels Bram can read**: when someone asks Bram
  ...
  Bram. They go to the AI provider as context for that one reply, and are not
  saved to Bram's transcript or personal memory merely because they were
  retrieved.
- **Messages staff teach to Bram**: staff can deliberately turn a server
  message into shared community knowledge or a reusable shared skill. The
  quoted message is sent to the AI provider for that task. What Bram learns may
  be stored in community memory or a shared skill. If the server configures a
  staff learn-log channel, Bram attempts to post a summary there; the log is
...
messages sent directly to Bram are ignored without being read into a turn,
stored in its transcript, or sent to an AI provider. Bram may also check whether
...
## How Bram uses your data, and who it's shared with

- **Answering you.** Your message and recent conversation are sent to the AI
  provider that powers Bram's replies. Like any cloud AI service, that provider
...
- **Safety checks.** If conversational content moderation is enabled, your
  message to Bram and Bram's draft reply may be checked by a moderation service
  before sending.
- **Custom personas.** If you ask Bram to adopt a persona, your request and
...
- **Internet search and page reading.** If enabled, Bram sends a search query,
...
- **X search.** If enabled, Bram sends your search query and any date or
...
- **Image generation and editing.** If enabled, Bram sends the image prompt,
...
- **Wolfram|Alpha.** If enabled, Bram sends a bounded, single-line computation
  query and optional units choice to Wolfram|Alpha.
- **Video understanding.** If enabled and you ask about a public YouTube video,
  Bram sends its URL and your questions to Google's paid Gemini API. If you
  select a Discord attachment or workspace video, Bram streams those video
...
Bram does **not** sell your data, use it for advertising, or share it outside
the configured services and tools needed to answer you.

## How long Bram keeps your data

- **Conversation history: 30 days.** Your messages to Bram (and any images in
...
- **Long-term memory: until you delete it.** If the server has long-term memory
  enabled, it is on for you by default, and Bram can remember durable facts you
...
- **Files you create or upload with Bram: 7 days.** Admitted current-message
...
- **Browser profile: 7 days.** If the interactive browser is enabled, the
  cookies, site storage, cache, history, and screenshots from tasks Bram does
  for you live in a profile that is private to you. It is removed after 7 days
  of inactivity, or sooner by **Delete my data**. The sites Bram opens on your
  behalf see that visit, and can set cookies that stay in your profile until it
  expires or you delete it.
- **Video sessions: up to 24 hours idle locally.** Bram removes local session
...
- **Personal skills: until you delete them.** Reusable instruction skills you
  create are stored separately from the expiring workspace. Ask Bram to delete
...
- **Bot blocks: until removed.** If you or a moderator block the bot from
  responding to you, Bram keeps your user ID plus the block's creator, reason,
...
- **Discord staff records: controlled by server staff.** Learning cards are
  ...
  not covered by Bram's local retention sweep or `/privacy`.
...
- **`/privacy` → Delete my data**: does everything above and also immediately
  deletes Bram's local copy of conversations you started, your messages in
  ...
  video sessions Bram also requests deletion of every
  ...
  conversation, Bram's local copy of that whole conversation is removed,
...
- **What `/privacy` cannot delete**: messages or files stored by Discord;
  ...
  the stored Gemini video Interactions Bram knows how to delete; diagnostic
...
session: your local deletion can finish while Bram is still retrying the
deletion request to Google in the background.

- **`/memory status`**: see whether memory is on for you.
- **`/memory opt-out`**: stop Bram from remembering anything new about you.
- **`/memory opt-in`**: turn memory back on after opting out.
- **Block Bram**: ask Bram to block you, and it will stop responding to you.
...
Bram is intended for users who are **old enough to use Discord in their
country, and never under 13**.

## Changes to this policy

We may update this policy as Bram changes. Material changes will be noted with
