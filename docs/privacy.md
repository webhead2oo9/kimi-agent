...
| `video_sessions`, `video_interactions` | Actor/root/guild-scoped public-YouTube specialist metadata: canonical URL/video id, model, opaque local and Gemini Interaction ids, counts, and timestamps. Questions and answers are not stored in these local tables. |
| `video_interaction_deletions` | Content-free retry rows for provider-side Interaction deletion: Gemini Interaction id, user id, timestamps, attempt count, and bounded last error. No URL, question, or answer. |
...
`video` tool, a `start` call sends the public YouTube URL and the user's specific
question to Google's paid Gemini API with `store=true`. Follow-ups use
`previous_interaction_id`, so Google stores the video-specialist chain and can
reuse it without the bot retransmitting the whole history. The normal Kimi chat
provider sees only the untrusted tool result it needs to answer.

Local sessions contain identifiers and scope metadata only and expire after at
most 24 hours of inactivity. Expiry, transcript retention, and full `/privacy`
deletion remove local access immediately and queue every known Gemini
Interaction id for Google's delete endpoint. The outbox is retried hourly and
on later privacy requests. If provider access is unavailable during `/privacy`,
the local deletion and privacy request complete, the user barrier is released,
and the provider deletion stays independently queued. Google documents
paid-tier Interaction retention of up to 55 days and may separately retain
limited safety/security records under its paid terms; local expiry is therefore
not a claim that every provider copy has already disappeared. See [Video understanding](video-understanding.md).
...
| Video specialist sessions | 24-hour maximum idle lifetime; local access is removed on expiry/root deletion and all known Gemini Interaction ids are queued for provider deletion. |
...
- **`/privacy`** (`commands/privacy_cmd.py`) shows the plain-language TL;DR
  ...
  - **Delete my data** first takes an exclusive per-user deletion lease. It
    ...
    Interaction and retries failures; it still cannot guarantee removal from
...
| Google Gemini video understanding | A public YouTube URL plus the user's video question and follow-ups; Google stores the Interaction chain for stateful continuation | `VIDEO_UNDERSTANDING_ENABLED` + `GEMINI_API_KEY` | off |
